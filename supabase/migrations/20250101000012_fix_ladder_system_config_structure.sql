-- Migration: Fix ladder_system_config table structure
-- This migration ensures all required columns exist in the ladder_system_config table

-- First, let's check if the table exists and drop it if it's incomplete
DROP TABLE IF EXISTS ladder_system_config CASCADE;

-- Recreate the ladder_system_config table with the correct structure
